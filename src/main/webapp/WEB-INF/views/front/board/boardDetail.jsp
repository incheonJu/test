...
<div id="scArea">
	<sf:form method="POST" modelAttribute="board" action="${pageContext.request.contextPath}/front/board/boardUpdateProcess" enctype="multipart/form-data">
	...
	<input type="hidden" name="page" value="${page}"/>
		...
		<security:authorize ifAnyGranted="ROLE_OPER">
			<img src="${pageContext.request.contextPath}/img/front/button/btn_edit.gif" onclick="location.href='${pageContext.request.contextPath}/front/board/boardUpdate/${board.boardSeq}';"  alt="수정"/> 
			<img src="${pageContext.request.contextPath}/img/front/button/btn_del.gif" onclick="$('#deleteDialog').dialog('open');"  alt="삭제"/>
		</security:authorize>
		<img id="listButton" src="${pageContext.request.contextPath}/img/front/button/btn_list.gif" width="57" height="23" 
				onclick="window.location.href = '${pageContext.request.contextPath}/front/board/boardList?page=${page}&searchString=${searchString}';"  alt="목록"/>
		</p>
	</sf:form>
</div>
...
$(function() {
	...
	$("#deleteDialog").dialog({
		...
		buttons : {
			확인 : function() {
				$(location).attr('href', '${pageContext.request.contextPath}/front/board/boardDelete/${board.boardSeq}');
				/* deleteData(); */
			},
			취소 : function() {
...
function fnSubmitReply() {
	if (fnCheckReply()) {
		var url = contextPath + "/front/board/boardReplyInsertProcess";
...
function setReplyData(replySeq){
	var url = contextPath + "/front/board/getBoardReply";
	var boardSeq = $("#boardSeq").val();
...
function fnUpdateReplyProcess(){
	if (fnCheckReply()){
		...
		var url = contextPath + "/front/board/updateBoardReplyProcess";
		var boardSeq = $("#boardSeq").val();
...
function fnDeleteReplyProcess(){
	if (fnCheckReply()){
		...
		var url = contextPath + "/front/board/deleteBoardReplyProcess";
		var boardSeq = $("#boardSeq").val();
		var replySeq = $("#replySeq").val();
...
function fnReloadReplyList(){
	var url = contextPath + "/front/board/getBoardReplyList";
	var boardSeq = $("#boardSeq").val();
