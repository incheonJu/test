<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="Content-Script-Type" content="text/javascript" />
<meta http-equiv="Content-Style-Type" content="text/css" />
<meta http-equiv="X-UA-Compatible" content="IE=edge" />
<meta charset="utf-8">
<title>EIP Template</title>
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/admin/style.css"/>
<link href="${pageContext.request.contextPath}/js/lib/jqGrid/css/ui.jqgrid.css" rel="stylesheet" type="text/css">
<link href="${pageContext.request.contextPath}/js/lib/jqueryUI/css/custom-theme/jquery-ui-1.10.3.custom.css" rel="stylesheet" type="text/css">
<style>
.button_10em { font-size: 1.0em }
.button_09em { font-size: 0.9em }
.button_07em { font-size: 0.7em }
</style>

<script type="text/javascript">
	var console = console || {
	    log:function(){},
	    warn:function(){},
	    error:function(){}
	};
</script>

<script src="${pageContext.request.contextPath}/js/lib/ckeditor/ckeditor.js"></script>
<script src="${pageContext.request.contextPath}/js/common/ServerAPICall.js"></script>
<script src="${pageContext.request.contextPath}/js/lib/jquery-1.10.1.min.js"></script>
<script src="${pageContext.request.contextPath}/js/lib/jquery-ui.js"></script>
<script language="JavaScript" type="text/javascript" src="${pageContext.request.contextPath}/js/common/common.js"></script>

<input type="hidden" id="contextPath" value="${pageContext.request.contextPath}" >