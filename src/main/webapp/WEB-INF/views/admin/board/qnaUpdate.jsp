...
<div id="scArea">
	<sf:form method="POST" modelAttribute="qna" action="${pageContext.request.contextPath}/admin/board/qnaUpdateProcess" enctype="multipart/form-data">
	...
	<input type="hidden" name="page" value="${page}"/>
		<table class="table_mem">
			<tr>
				...
				<td colspan="3">
					<input id="qnaTitle" name="qnaTitle" size="100" maxlength="100" value="${qna.qnaTitle}" />
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="3">
					...
					<input type="hidden" id="qnaEtc" name="qnaEtc">
				</td>
			</tr>
			
			<c:forEach var="qnaFile" items="${qna.qnaFileList}" varStatus="status">
				<tr>
					<c:if test="${status.count == 1}">
						<th width="15%" scope="col" rowspan="${qna.qnaFileList.size()}">첨부파일<br/><br/>(파일 최대 크기 20MB)</th>
						<td colspan="2" width="250">
					</c:if>
					<c:if test="${status.count != 1}">
						<td colspan="2" width="250">
					</c:if>
						<c:if test="${(qnaFile.fileName != null) && (qnaFile.fileName != '')}">
							<a href="${pageContext.request.contextPath}/download/Image/board/${qnaFile.fileName}" target="_blank">${qnaFile.fileName}</a> <input type="checkbox" value="${qnaFile.fileSeq}" name="fileDel" id="fileDel${qnaFile.fileSeq}" onclick="fnFileDelClick(this,'${qnaFile.fileSeq}');"/>삭제
						</c:if>
						</td>
						<td colspan="1">
							<input name="fileUpdate" type="file" class="inputfile" id="fileUpdate${qnaFile.fileSeq}" size="50" onchange="fnFileUpload('${qnaFile.fileSeq}');"/> 
							<input name="fileUpdateSeq" type="hidden" class="inputfile" id="fileUpdateSeq${qnaFile.fileSeq}" size="50" />
						</td>
				</tr>
			</c:forEach>
			<tr>
