...
<div id="scArea">
	...
	<security:authorize ifAnyGranted="ROLE_OPER" >
		<p class="joinBtn">
			<img src="${pageContext.request.contextPath}/img/admin/button/btn_add.gif" onclick="$(location).attr('href', '${pageContext.request.contextPath}/admin/board/boardInsert')"  alt="등록"/>
		</p>
	</security:authorize>
</div>
	...
	<script type="text/javascript">
		...
		function jqGrid(url, pageNum) {
			...
			jQuery("#list").jqGrid({
				...
				colNames : [ 'No', '제목', '등록일' ],
				colModel : [
					{name : 'boardSeq', index : 'BOARD_SEQ', width : 80, align : "left"}, 
					{name : 'boardTitle', index : 'BOARD_TITLE', width : 430, align : "left", formatter:titleFormat},
				...
				onSelectRow : function(rowid, iRow, iCol, e) {
					...
					$(location).attr('href', "${pageContext.request.contextPath}/admin/board/boardDetail/" + seq + "?page="+$("#list").getGridParam('page')+"&searchString=" + searchString);
				},
				caption : "공지사항"
			});
		...
		{
			var title = rowObject.boardTitle;
		...
		function searchDataGrid(searchField, searchString, pageNum){
			...
			var url = "${pageContext.request.contextPath}/admin/board/boardJqList?searchField=" + searchField + "&searchString=" + searchString;
