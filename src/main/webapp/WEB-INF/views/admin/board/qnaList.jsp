...
<div id="scArea">
	...
	<security:authorize ifAnyGranted="ROLE_OPER" >
		<p class="joinBtn">
			<img src="${pageContext.request.contextPath}/img/admin/button/btn_add.gif" onclick="$(location).attr('href', '${pageContext.request.contextPath}/admin/board/qnaInsert')"  alt="등록"/>
		</p>
	</security:authorize>
</div>
	...
	<script type="text/javascript">
		...
		function jqGrid(url, pageNum) {
			...
			jQuery("#list").jqGrid({
				...
				autowidth : true,
				height: "100%",
				colNames : [ 'No', '제목', '이름', '공개여부', '답변여부', '등록일' ],
				colModel : [
					{name : 'qnaSeq', index : 'QNA_SEQ', width : 80, align : "left"}, 
					{name : 'qnaTitle', index : 'QNA_TITLE', width : 430, align : "left", formatter:titleFormat},
					{name : 'createName', index : 'CREATE_NAME', width : 100, align : "left"},
					{name : 'openYn', index : 'OPEN_YN', width : 80, align : "left"},
					{name : 'replyYn', index : 'REPLY_YN', width : 80, align : "left"},
				...
				onSelectRow : function(rowid, iRow, iCol, e) {
					...
					$(location).attr('href', "${pageContext.request.contextPath}/admin/board/qnaDetail/" + seq + "?page="+$("#list").getGridParam('page')+"&searchString=" + searchString);
				},
				caption : "Q&A"
			});
		...
		function searchDataGrid(searchField, searchString, pageNum){
			...
			var url = "${pageContext.request.contextPath}/admin/board/qnaJqList?searchField=" + searchField + "&searchString=" + searchString;
