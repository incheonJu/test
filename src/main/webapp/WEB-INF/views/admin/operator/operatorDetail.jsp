...
<div class="stitle">
	<h3>처리자 회원상세보기(관리자용)</h3>
</div>

<div id="scArea">
	<sf:form method="POST" modelAttribute="operator" action="${pageContext.request.contextPath}/admin/operator/updateProcess" >
		<input type="hidden" name="page" value="${page}">
		<sf:hidden path="operPwd"/>
		...
		<table class="table_mem">
			<tr>
				...
				<td colspan="3">
					<sf:input path="operId" size="30" readonly="true" />
				</td>
			</tr>
			<tr>
				<th scope="col">이름</th>
				<td colspan="3">
					<sf:input path="operName" size="30" />
				</td>
			</tr>
			<tr>
				<th scope="col">이메일</th>
				<td colspan="3">
					...
					<c:set var="arrEmail" value="${fn:split(operator.email,'@')}"/>
			...
			<tr>
				...
				<td colspan="3">
					<input type="radio" name="operState" id="operState" class="chkbox" value="0" <c:if test="${operator.operState == '0'}">checked="checked"</c:if>>
					<label for="operState">승인 대기</label>&nbsp;&nbsp;
					<input type="radio" name="operState" id="operState" class="chkbox" value="1" <c:if test="${operator.operState == '1'}">checked="checked"</c:if>>
					<label for="operState">승인</label>&nbsp;&nbsp; 
				</td>
			</tr>
			<tr>
				<th scope="col">권한</th>
				<td colspan="3">
					<input type="checkbox" name="priv01" id="priv01" class="chkbox" value="${operator.priv01}" <c:if test="${operator.priv01 == true}">checked="checked"</c:if>>
					<label for="priv1">1차권한</label>&nbsp;&nbsp;
					<input type="checkbox" name="priv02" id="priv02" class="chkbox" value="${operator.priv02}" <c:if test="${operator.priv02 == true}">checked="checked"</c:if>>
					<label for="priv2">2차권한</label>&nbsp;&nbsp; 
					<input type="checkbox" name="priv03" id="priv03" class="chkbox" value="${operator.priv03}" <c:if test="${operator.priv03 == true}">checked="checked"</c:if>>
					<label for="priv3">3차권한</label>
					<input type="checkbox" name="priv04" id="priv04" class="chkbox" value="${operator.priv04}" <c:if test="${operator.priv04 == true}">checked="checked"</c:if>>
					<label for="priv4">4차권한</label>
					<input type="checkbox" name="priv05" id="priv05" class="chkbox" value="${operator.priv05}" <c:if test="${operator.priv05 == true}">checked="checked"</c:if>>
					<label for="priv5">5차권한</label>
				</td>
			</tr>
		</table>
		<br/>
...
<div id="modifyDialog" title="수정 알림">
	<table class="table_mem">
		<tr>
			<th height="50">해당 처리자 정보를 수정하시겠습니까?</th>
		</tr>
	</table>
</div>
...
<div id="deleteDialog" title="삭제 확인">
	<table class="table_mem">
		<tr>
			<th height="50">해당 처리자를 삭제(탈퇴처리)하시겠습니까?</th>
		</tr>
	</table>
</div>
...
<script type="text/javascript">
	$(function() {
		...
		$("#deleteDialog").dialog({
			...
			buttons : {
				확인 : function() {
					$(location).attr('href', '${pageContext.request.contextPath}/admin/operator/delete/${operId}');
				},
				취소 : function() {
	...
	function submit()
	{
		if (formDataCheck())
		{
			...
			if ($("#priv01").is(":checked")) $("#priv01").val("true");
			if ($("#priv02").is(":checked")) $("#priv02").val("true");
			if ($("#priv03").is(":checked")) $("#priv03").val("true");
			if ($("#priv04").is(":checked")) $("#priv04").val("true");
			if ($("#priv05").is(":checked")) $("#priv05").val("true");
