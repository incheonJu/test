...
<div id="scArea">
	<sf:form method="POST" modelAttribute="operator" action="${pageContext.request.contextPath}/admin/updateMyinfoProcess" >
		...
		<table class="table_mem">
			...
			<tr>
				<th scope="col">이름</th>
				<td colspan="3">
					<sf:input path="operName" size="30" readonly="true"/>
				</td>
			</tr>
			<tr>
				<th scope="col">현재 비밀번호</th>
				<td><input name="operPwd" type="password" id="operPwd" size="30" />
			</tr>
			<tr>
				<th scope="col">새로운 비밀번호</th>
				<td><input name="newOperPwd" type="password" id="newOperPwd" size="30" onkeyup="checkPwd()"/><span id="idPwdResult">비빌번호 변경시에만 입력하세요. 비밀번호는 6글자 이상입니다.</span>
			</tr>
			<tr>
				<th scope="col">비밀번호 확인</th>
				<td><input name="newOperPwd2" type="password" id="newOperPwd2" size="30" onkeyup="checkPwd2()"/><span id="idPwdConfrimResult"></span>
			</tr>
			<tr>
				<th scope="col">이메일</th>
				<td colspan="3">
...
<script type="text/javascript">
	...
	function formDataCheck()
	{
		...
		console.log(operPwd.val().trim().length);
		
		if (operPwd.val().trim().length <= 0) {
			alert("비밀번호를 입력해 주세요");	
			focus = operPwd;
			return false;
			...
			focus = tel1;
			return false;
		} else if ( $('#newOperPwd').val() != '' && (!checkPwd() || !checkPwd2())) { // 비밀번호 변경시...
